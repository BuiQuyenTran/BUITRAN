...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {
    ...
    for (int i=0;i<source->offices[source_office_index].packages_count;i++){
        ...
        if (min <=mxg && max >=mxg){
            ...
            target->offices[target_office_index].packages= (package*) realloc (target->offices[target_office_index].packages, sizeof (package)* (count +1));
            ...
            source->offices[source_office_index].packages= (package*)realloc (source->offices[source_office_index].packages, sizeof (package)* (count-1));
...
town town_with_most_packages(town* towns, int towns_count) {
    ...
    while (--towns_count>-1){
        sum=0;
        for (int j=0;j<towns[towns_count].offices_count;j++){
            sum+=towns[towns_count].offices[j].packages_count;
        }
        if (sum > max){
            max=sum;
            r=towns_count;
...
{
    int towns_count;
    freopen ("post_transition.txt", "r", stdin);
    scanf("%d", &towns_count);
